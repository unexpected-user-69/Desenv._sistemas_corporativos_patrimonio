...
export class ProxyService {
  ...
  private readonly services: Map<string, ServiceConfig> = new Map([
    ...
    ['events', {
      name: 'events-service',
      baseUrl: process.env.EVENTS_SERVICE_URL || 'http://localhost:3003',
      timeout: 10000,
    }],
    ['audit', {
      name: 'audit-service',
      baseUrl: process.env.AUDIT_SERVICE_URL || 'http://localhost:3004',
      timeout: 10000,
    }],
    ['categorias', {
      name: 'categorias-service',
      baseUrl: process.env.CATEGORIAS_SERVICE_URL || 'http://localhost:3005',
      timeout: 10000,
    }],
    ['patrimonio', {
      name: 'patrimonio-service',
      baseUrl: process.env.PATRIMONIO_SERVICE_URL || 'http://localhost:3006',
      timeout: 10000,
    }],
  ]);
