...
import { PatrimonioLocalizacaoHistorico } from '../patrimonio/entities/patrimonio-localizacao-historico.entity';
config();

const dbSchema = process.env.DB_SCHEMA ?? 'patrimonio';

const common = {
  type: 'postgres' as const,
  entities: [Patrimonio, PatrimonioLocalizacaoHistorico],
  migrations: [__dirname + '/migrations/*.{ts,js}'],
  synchronize: false, // nunca em prod
  migrationsRun: false, // rode pelos scripts
  logging: process.env.DB_LOGGING === 'true',
  applicationName: 'patrimonio-service',
  schema: dbSchema, // Schema isolado para patrimonio-service
