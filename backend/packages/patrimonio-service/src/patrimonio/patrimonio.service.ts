...
export class PatrimonioService {
  ...
  constructor(
    ...
    private readonly categoriasHttpClient: CategoriasHttpClient,
  ...
  async getHistoricoResponsaveis(
    ...
    // Se tem responsável atual, buscar informações via HTTP client
    if (patrimonio.responsavelId) {
      try {
        const user = await this.usersHttpClient.findOne(patrimonio.responsavelId);
        responsaveis.push({
          responsavelId: patrimonio.responsavelId,
          responsavelNome: user?.name || undefined,
          dataInicio: patrimonio.createdAt,
          observacoes: patrimonio.observacoes || undefined,
        });
      } catch (error) {
        // Se não conseguir buscar o usuário, adicionar sem nome
        responsaveis.push({
          responsavelId: patrimonio.responsavelId,
  ...
  async getStatsByResponsavel(responsavelId: string): Promise<ResponsavelStatsResponseDto> {
    ...
    // Agrupar por categoria (buscar nomes via HTTP client)
    const porCategoria: Record<string, number> = {};
    const categoriaIds = [...new Set(patrimonios.map(p => p.categoriaId).filter(Boolean))];
    ...
    for (const categoriaId of categoriaIds) {
      if (categoriaId) {
        try {
          const categoria = await this.categoriasHttpClient.findOne(categoriaId);
          if (categoria) {
            categoriasMap.set(categoriaId, categoria.nome);
          }
        } catch (error) {
          this.logger.warn(`Erro ao buscar categoria ${categoriaId}:`, error);
        }
      }
    }
    
    patrimonios.forEach((p) => {
      const categoriaNome = p.categoriaId ? (categoriasMap.get(p.categoriaId) || 'SEM_CATEGORIA') : 'SEM_CATEGORIA';
