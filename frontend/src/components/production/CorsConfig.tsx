...
export const CorsConfig: React.FC<Props> = ({ config, onUpdate }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      setMessage({ type: 'success', text: 'Configuração de CORS salva com sucesso!' });
      onUpdate();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-gray-900">Configuração de CORS</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center">
              <input
                ...
                onChange={(e) => setFormData({ ...formData, credentials: e.target.checked })}
                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
              />
              <label htmlFor="credentials" className="ml-2 block text-sm text-gray-700">
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                Tempo que o navegador pode cachear a resposta do preflight (0 = sem cache)
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-blue-50 rounded-lg p-4">
            <h3 className="font-medium text-blue-900 mb-2">Informações sobre CORS</h3>
            <div className="text-sm text-blue-800 space-y-2">
              <p><strong>Origin:</strong> Domínios que podem acessar a API</p>
              <p><strong>Métodos:</strong> Verbos HTTP permitidos</p>
              <p><strong>Headers:</strong> Cabeçalhos permitidos nas requisições</p>
              <p><strong>Credentials:</strong> Permite envio de cookies e auth headers</p>
              <p><strong>Max Age:</strong> Cache da resposta preflight</p>
            </div>
          </div>
          ...
          <div className="bg-yellow-50 rounded-lg p-4">
            <h3 className="font-medium text-yellow-900 mb-2">Recomendações de Segurança</h3>
