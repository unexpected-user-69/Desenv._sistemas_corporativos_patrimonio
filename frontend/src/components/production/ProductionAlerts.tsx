...
export const ProductionAlerts: React.FC<Props> = ({ alerts, onResolve }) => {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'cors_violation': return '🌐';
      case 'security_threat': return '🔒';
      case 'compression_error': return '🗜️';
  ...
  const getTypeName = (type: string) => {
    switch (type) {
      ...
      case 'cors_violation': return 'Violação CORS';
      case 'security_threat': return 'Ameaça de Segurança';
      case 'compression_error': return 'Erro de Compressão';
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-gray-900">Alertas de Produção</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            >
              ...
              <option value="unresolved">Não Resolvidos ({alerts.filter(a => !a.resolved).length})</option>
              <option value="resolved">Resolvidos ({alerts.filter(a => a.resolved).length})</option>
            </select>
          </div>
                ...
                <div className="flex-1">
                  ...
                  {alert.metadata && (
                    <details className="mt-3">
                      <summary className="text-xs text-gray-600 cursor-pointer">Detalhes Técnicos</summary>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Estatísticas de Alertas</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-orange-600">{alerts.filter(a => a.severity === 'critical' || a.severity === 'high').length}</p>
            <p className="text-sm text-gray-600">Alta Prioridade</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-blue-600">{alerts.filter(a => a.type === 'security_threat').length}</p>
