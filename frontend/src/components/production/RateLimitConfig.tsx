...
export const RateLimitConfig: React.FC<Props> = ({ config, onUpdate }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      setMessage({ type: 'success', text: 'Configuração de Rate Limiting salva com sucesso!' });
      onUpdate();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-gray-900">Configuração de Rate Limiting</h2>
        <p className="text-sm text-gray-600">
          Configure os limites de requisições para proteção contra spam e ataques DDoS
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Tempo adicional de bloqueio após exceder o limite (0 = sem bloqueio extra)
            </p>
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, keyGenerator: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
          ...
          <div className="space-y-3">
            <div className="flex items-center">
              <input
                ...
                onChange={(e) => setFormData({ ...formData, skipSuccessfulRequests: e.target.checked })}
                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
              />
              <label htmlFor="skipSuccessfulRequests" className="ml-2 block text-sm text-gray-700">
            ...
            <div className="flex items-center">
              <input
                ...
                onChange={(e) => setFormData({ ...formData, skipFailedRequests: e.target.checked })}
                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
              />
              <label htmlFor="skipFailedRequests" className="ml-2 block text-sm text-gray-700">
        ...
        <div className="space-y-4">
          <div className="bg-blue-50 rounded-lg p-4">
            <h3 className="font-medium text-blue-900 mb-2">Informações sobre Rate Limiting</h3>
            <div className="text-sm text-blue-800 space-y-2">
              <p><strong>TTL:</strong> Janela de tempo para contagem de requisições</p>
              <p><strong>Limite:</strong> Número máximo de requisições na janela</p>
              <p><strong>Bloqueio:</strong> Tempo adicional após exceder o limite</p>
              <p><strong>Gerador:</strong> Como identificar requisições únicas</p>
            </div>
          </div>
