...
export const RequestLogs: React.FC<Props> = ({ logs }) => {
  ...
  const [sortBy, setSortBy] = useState<'timestamp' | 'duration' | 'statusCode'>(
    'timestamp',
  );
  ...
  const filteredAndSortedLogs = logs
    .filter((log) => {
      ...
      if (
        ...
        log.statusCode.toString() !== filter.statusCode
      )
        return false;
      if (
        filter.search &&
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFilter({ ...filter, statusCode: e.target.value })
              }
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              <option value="all">Todos</option>
          ...
          <div>
            ...
            <div className="flex space-x-2">
              <select
                ...
                onChange={(e) =>
                  setSortBy(
                    e.target.value as 'statusCode' | 'timestamp' | 'duration',
                  )
                }
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={() =>
              setFilter({ method: 'all', statusCode: 'all', search: '' })
            }
              ...
              {filteredAndSortedLogs.map((log) => (
                <tr key={log.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getMethodColor(log.method)}`}
                    >
                      {log.method}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(log.statusCode)}`}
                    >
                      {log.statusCode}
                    </span>
                  </td>
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-sm text-gray-900 bg-gray-100 p-2 rounded">
                      ...
                      <p>
                        Reset:{' '}
                        {new Date(
                          selectedLog.rateLimitStatus.resetTime,
                        ).toLocaleString()}
                      </p>
                    </div>
                  </div>
                ...
                {selectedLog.compressionApplied && (
                  <div>
                    ...
                    <div className="mt-1 text-sm text-gray-900 bg-gray-100 p-2 rounded">
                      <p>
                        Compressão aplicada:{' '}
                        {selectedLog.compressionApplied ? 'Sim' : 'Não'}
                      </p>
                      {selectedLog.compressionRatio && (
                        <p>
                          Taxa de compressão:{' '}
                          {(selectedLog.compressionRatio * 100).toFixed(1)}%
                        </p>
                      )}
                    </div>
                  </div>
