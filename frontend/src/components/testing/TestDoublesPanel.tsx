...
export const TestDoublesPanel: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'list' | 'create' | 'examples'>(
    'list',
  );

  useEffect(() => {
    void fetchTestDoubles();
  }, []);
  ...
  const fetchTestDoubles = async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Erro ao carregar test doubles',
      );
  ...
  const getTypeDescription = (type: string) => {
    switch (type) {
      case 'dummy':
        return 'Objeto que nunca é usado, apenas preenche parâmetros';
      case 'stub':
        return 'Fornece respostas pré-definidas para chamadas de método';
      case 'spy':
        return 'Registra informações sobre como foi chamado';
      case 'mock':
        return 'Verifica se as interações ocorreram como esperado';
      case 'fake':
        return 'Implementação funcional simplificada para testes';
      default:
        return 'Tipo de test double não reconhecido';
    }
  };
          ...
          {testDoubles.map((testDouble) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium ${getTypeColor(testDouble.type)}`}
                >
              ...
              <p className="text-xs text-gray-500 mb-4">
                {getTypeDescription(testDouble.type)}
              </p>
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  <span
                    className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getTypeColor(selectedDouble.type)}`}
                  >
                    {selectedDouble.type.toUpperCase()}
                  </span>
                  <p className="text-sm text-gray-600 mt-2">
                    {selectedDouble.description}
                  </p>
                </div>
