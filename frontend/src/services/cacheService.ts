...
class CacheService {
  ...
  async getStats(): Promise<CacheStats> {
    try {
      const response = await this.api.get<CacheStats>('/v1/cache/stats');
  ...
  async getKeys(
    ...
    try {
      const response = await this.api.get<CacheKey[]>('/v1/cache/keys', {
  ...
  async getConfig(): Promise<CacheConfig> {
    try {
      const response = await this.api.get<CacheConfig>('/v1/cache/config');
      return response.data;
    } catch (error) {
  ...
  async updateConfig(config: Partial<CacheConfig>): Promise<CacheConfig> {
    try {
      const response = await this.api.patch<CacheConfig>(
        '/v1/cache/config',
        config,
      );
      return response.data;
  ...
  async getOperations(limit: number = 50): Promise<CacheOperation[]> {
    try {
      const response = await this.api.get<CacheOperation[]>(
        '/v1/cache/operations',
        {
          params: { limit },
        },
  ...
  async getMetrics(): Promise<CacheMetrics> {
    try {
      const response = await this.api.get<CacheMetrics>('/v1/cache/metrics');
      return response.data;
    } catch (error) {
  ...
  async getAlerts(): Promise<CacheAlert[]> {
    try {
      const response = await this.api.get<CacheAlert[]>('/v1/cache/alerts');
      return response.data;
    } catch (error) {
  ...
  async getHealth(): Promise<CacheHealth> {
    try {
      const response = await this.api.get<CacheHealth>('/v1/cache/health');
      return response.data;
    } catch (error) {
  ...
  async getPatterns(): Promise<CachePattern[]> {
    try {
      const response = await this.api.get<CachePattern[]>('/v1/cache/patterns');
      return response.data;
    } catch (error) {
  ...
  async searchKeys(options: CacheSearchOptions): Promise<CacheSearchResult> {
    try {
      const response = await this.api.post<CacheSearchResult>(
        '/v1/cache/search',
        options,
      );
      return response.data;
