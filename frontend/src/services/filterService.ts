...
class FilterService {
  ...
  async getFilterSuggestions(field: string, query: string): Promise<FilterSuggestion[]> {
  ...
  async createFilterPreset(preset: Omit<FilterPreset, 'id' | 'createdAt' | 'updatedAt'>): Promise<FilterPreset> {
    const response = await this.api.post('/v1/filters/presets', preset);
    return response.data;
  }

  async updateFilterPreset(id: string, preset: Partial<FilterPreset>): Promise<FilterPreset> {
  ...
  async getFilterPerformance(filters: AdvancedFilters): Promise<FilterPerformance> {
  ...
  async getCachedFilterResults(filters: AdvancedFilters): Promise<any> {
    const cacheKey = this.buildQueryString(filters);
    return this.filterCache.get(cacheKey);
  }

  setCachedFilterResults(filters: AdvancedFilters, results: any): void {
    const cacheKey = this.buildQueryString(filters);
  ...
  async getFilterRecommendations(currentFilters: AdvancedFilters): Promise<{
    ...
    const response = await this.api.post('/v1/filters/recommendations', currentFilters);
    return response.data;
  }

  // Métodos de exportação
  async exportFilterResults(filters: AdvancedFilters, format: 'csv' | 'json' | 'xlsx'): Promise<Blob> {
    const response = await this.api.post('/v1/filters/export', {
      filters,
      format
    }, {
      responseType: 'blob'
  ...
  async subscribeToFilterStats(callback: (stats: FilterAnalytics) => void): Promise<() => void> {
