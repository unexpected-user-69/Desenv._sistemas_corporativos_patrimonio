...
"@
    ...
    Write-Host "✅ Arquivo .env.prod criado!" -ForegroundColor Green
    Write-Host ""
}
...
if ($LASTEXITCODE -ne 0) { Write-Host "⚠️  Falha ao baixar users-service" -ForegroundColor Yellow }

docker pull "ghcr.io/$RepoOwner/auth-service:$DefaultTag"
if ($LASTEXITCODE -ne 0) { Write-Host "⚠️  Falha ao baixar auth-service" -ForegroundColor Yellow }

docker pull "ghcr.io/$RepoOwner/patrimonio-service:$DefaultTag"
if ($LASTEXITCODE -ne 0) { Write-Host "⚠️  Falha ao baixar patrimonio-service" -ForegroundColor Yellow }

docker pull "ghcr.io/$RepoOwner/categorias-service:$DefaultTag"
if ($LASTEXITCODE -ne 0) { Write-Host "⚠️  Falha ao baixar categorias-service" -ForegroundColor Yellow }

docker pull "ghcr.io/$RepoOwner/audit-service:$DefaultTag"
if ($LASTEXITCODE -ne 0) { Write-Host "⚠️  Falha ao baixar audit-service" -ForegroundColor Yellow }

docker pull "ghcr.io/$RepoOwner/events-service:$DefaultTag"
if ($LASTEXITCODE -ne 0) { Write-Host "⚠️  Falha ao baixar events-service" -ForegroundColor Yellow }

docker pull "ghcr.io/$RepoOwner/api-gateway:$DefaultTag"
if ($LASTEXITCODE -ne 0) { Write-Host "⚠️  Falha ao baixar api-gateway" -ForegroundColor Yellow }
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Erro ao subir containers. Verifique os logs." -ForegroundColor Red
    exit 1
}

Write-Host ""
Write-Host "⏳ Aguardando serviços inicializarem (aguarde ~30 segundos)..." -ForegroundColor Yellow
